...
class CompromissoFactory:
    ...
    @classmethod
    def editar(cls, compromisso: Compromisso) -> None:
        """
        ...
        # Se for marcado como 'Concluído', mas o usuário não informar Data e Hora da Conclusão,
        # insere a data e hora atual automaticamente.
        if compromisso.concluido is True and (compromisso.data_conclusao is None or compromisso.hora_conclusao is None):
            agora = datetime.now()
            compromisso.data_conclusao = agora.date()
            compromisso.hora_conclusao = agora.time().replace(second=0, microsecond=0)   

    @classmethod
    def concluir(cls, compromisso: Compromisso) -> None:
        """
